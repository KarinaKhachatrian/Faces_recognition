...
import pickle
def train_model_img1():

    if not os.path.exists("Питт"):
        print("Данная директория не существует")
        sys.exit()

    name = 'Pitt'
    known_encodings = []
    images = os.listdir("Питт") #сохраняем список из всех доступных изображений

    for(i, image) in enumerate(images):
        print(f"[+] processing img {i + 1}/{len(images)}")

        face_img = face_recognition.load_image_file(f"Питт/{image}")
