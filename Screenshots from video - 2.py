...
def take_screen_from_video():
    cap = cv2.VideoCapture("Бред Питт.mp4")
    count = 0

    if not os.path.exists("dataset_from_video_1"):
        os.mkdir("dataset_from_video_1")
    ...
    while True:
        ...
        if ret:
            ...
            if frame_id % multiplier == 0:
                cv2.imwrite(f"dataset_from_video_1/{count}.jpg", frame)
                print(f"Сделан скриншот {count}")
                count += 1

            if k == ord(" "):
                cv2.imwrite(f"dataset_from_video_1/{count}_extrascreen.jpg", frame)
